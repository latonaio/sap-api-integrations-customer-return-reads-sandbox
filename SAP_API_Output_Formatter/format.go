...
import (
	"encoding/json"
	"sap-api-integrations-customer-return-reads/SAP_API_Caller/responses"
...
func ConvertToHeader(raw []byte, l *logger.Logger) ([]Header, error) {
	...
	for i := 0; i < 10 && i < len(pm.D.Results); i++ {
		...
		header = append(header, Header{
			...
			CustomerTaxClassification5:    data.CustomerTaxClassification5,
			CustomerTaxClassification6:    data.CustomerTaxClassification6,
			CustomerTaxClassification7:    data.CustomerTaxClassification7,
			CustomerTaxClassification8:    data.CustomerTaxClassification8,
			CustomerTaxClassification9:    data.CustomerTaxClassification9,
...
func ConvertToToHeaderPartner(raw []byte, l *logger.Logger) ([]ToHeaderPartner, error) {
	...
	for i := 0; i < 10 && i < len(pm.D.Results); i++ {
		...
		toHeaderPartner = append(toHeaderPartner, ToHeaderPartner{
			...
			BusinessPartnerAddressUUID:   data.BusinessPartnerAddressUUID,
			BPRefAddrForDocSpcfcAddrUUID: data.BPRefAddrForDocSpcfcAddrUUID,
		})
	}
